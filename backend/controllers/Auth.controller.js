...
const sendResetPasswordMail = async (email, token) => {
  const transporter = nodemailer.createTransport({
    ...
    port: 587,
    auth: {
      user: 'keanu.schamberger@ethereal.email',
      pass: '2sSn3V9EC91nRyKWVM',
    },
  });
  const mailOptions = {
    from: 'keanu.schamberger@ethereal.email',
    to: email,
    subject: 'Reset Password',
    html: `<p> Please copy the link and <a href="http://localhost:3000/reset_password?token=${token}">reset your password</a>.</p>`,
  };
